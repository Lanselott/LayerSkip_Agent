...
def prepare_cnn_dm_summarization_format(n_shot: int = 0, seed: int = 42, template: str = None) -> List[
    EvaluationExample]:
    prompt_shots = ""
    if n_shot > 0:
...
def prepare_mmlu(template: str = None) -> List[EvaluationExample]:
    evaluation_data_points = []
    # TODO: Update prompt
    for data_point in load_dataset('cais/mmlu', 'all', split='test'):
        prompt = apply_template(message=data_point["question"], template=template)
...
def prepare_custom(data_path: str, prompt_field: str = "prompt", response_field: str = "response",
                   template: str = None) -> List[EvaluationExample]:
    evaluation_data_points = []
...
def get_data(
    ...
    if dataset == DatasetFormat.CHAT_FORMAT:
        ...
        evaluation_data_points = prepare_mmlu(template)
